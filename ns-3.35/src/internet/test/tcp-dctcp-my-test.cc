 ...
 *
 * This file is a duplicate of `tcp-dctcp-test.cc`.
 *
 */
...
NS_LOG_COMPONENT_DEFINE ("TcpDctcpMyTestSuite");
 ...
 */
class TcpDctcpMyCodePointsTest : public TcpGeneralTest
{
public:
  /**
   * \brief Constructor
   ...
   */
  TcpDctcpMyCodePointsTest (uint8_t testCase, const std::string &desc);
...
TcpDctcpMyCodePointsTest::TcpDctcpMyCodePointsTest (uint8_t testCase, const std::string &desc)
  : TcpGeneralTest (desc),
    m_senderSent (0),
    m_receiverSent (0),
    m_senderReceived (0),
...
void
TcpDctcpMyCodePointsTest::Tx (const Ptr<const Packet> p, const TcpHeader &h, SocketWho who)
{
...
void
TcpDctcpMyCodePointsTest::Rx (const Ptr<const Packet> p, const TcpHeader &h, SocketWho who)
{
...
void
TcpDctcpMyCodePointsTest::ConfigureProperties ()
{
 ...
 *
 */
class TcpDctcpMyCongestedRouter : public TcpSocketMsgBase
{
public:
  /**
   ...
   */
  TcpDctcpMyCongestedRouter (const TcpDctcpMyCongestedRouter &other)
...
NS_OBJECT_ENSURE_REGISTERED (TcpDctcpMyCongestedRouter);

TypeId
TcpDctcpMyCongestedRouter::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::TcpDctcpMyCongestedRouter")
    .SetParent<TcpSocketMsgBase> ()
    .SetGroupName ("Internet")
    .AddConstructor<TcpDctcpMyCongestedRouter> ()
  ;
  return tid;
}

void
TcpDctcpMyCongestedRouter::ReTxTimeout ()
{
  TcpSocketBase::ReTxTimeout ();
}

void
TcpDctcpMyCongestedRouter::SetTestCase (uint8_t testCase)
{
  m_testCase = testCase;
}

uint32_t
TcpDctcpMyCongestedRouter::SendDataPacket (SequenceNumber32 seq, uint32_t maxSize, bool withAck)
{
  ...
  if (m_retxEvent.IsExpired ())
    {
      ...
      m_retxEvent = Simulator::Schedule (m_rto, &TcpDctcpMyCongestedRouter::ReTxTimeout, this);
    }
  ...
  if (seq + sz > m_tcb->m_highTxMark)
    {
      Simulator::ScheduleNow (&TcpDctcpMyCongestedRouter::NotifyDataSent, this,
...
Ptr<TcpSocketBase>
TcpDctcpMyCongestedRouter::Fork (void)
{
  return CopyObject<TcpDctcpMyCongestedRouter> (this);
}

Ptr<TcpSocketMsgBase>
TcpDctcpMyCodePointsTest::CreateSenderSocket (Ptr<Node> node)
{
  if (m_testCase == 2)
    {
    ...
    {
      Ptr<TcpDctcpMyCongestedRouter> socket = DynamicCast<TcpDctcpMyCongestedRouter> (
          CreateSocket (node,
                        TcpDctcpMyCongestedRouter::GetTypeId (),
                        TcpDctcpMy::GetTypeId ()));
      socket->SetTestCase (m_testCase);
      return socket;
    }
  else
    {
      return TcpGeneralTest::CreateSocket (node, TcpSocketMsgBase::GetTypeId (), TcpDctcpMy::GetTypeId ());
    }
}

Ptr<TcpSocketMsgBase>
TcpDctcpMyCodePointsTest::CreateReceiverSocket (Ptr<Node> node)
{
  if (m_testCase == 2)
    {
 ...
 */
class TcpDctcpMyToLinuxReno : public TestCase
{
public:
  /**
   * \brief Constructor
   ...
   */
  TcpDctcpMyToLinuxReno (uint32_t cWnd, uint32_t segmentSize, uint32_t ssThresh,
...
TcpDctcpMyToLinuxReno::TcpDctcpMyToLinuxReno (uint32_t cWnd, uint32_t segmentSize, uint32_t ssThresh,
                                      uint32_t segmentsAcked, SequenceNumber32 highTxMark,
                                      SequenceNumber32 lastAckedSeq, Time rtt, const std::string &name)
  : TestCase (name),
    m_cWnd (cWnd),
...
void
TcpDctcpMyToLinuxReno::DoRun ()
{
  Simulator::Schedule (Seconds (0.0), &TcpDctcpMyToLinuxReno::ExecuteTest, this);
  Simulator::Run ();
  Simulator::Destroy ();
}

void
TcpDctcpMyToLinuxReno::ExecuteTest ()
{
  ...
  Ptr<TcpDctcpMy> cong = CreateObject <TcpDctcpMy> ();
 ...
 */
class TcpDctcpMyTestSuite : public TestSuite
{
public:
  TcpDctcpMyTestSuite () : TestSuite ("tcp-dctcp-my-test", UNIT)
  {
    AddTestCase (new TcpDctcpMyToLinuxReno (2 * 1446, 1446, 4 * 1446, 2, SequenceNumber32 (4753), SequenceNumber32 (3216), MilliSeconds (100), "DCTCP falls to New Reno for slowstart"), TestCase::QUICK);
    AddTestCase (new TcpDctcpMyCodePointsTest (1, "ECT Test : Check if ECT is set on Syn, Syn+Ack, Ack and Data packets for DCTCP packets"),
                 TestCase::QUICK);
    AddTestCase (new TcpDctcpMyCodePointsTest (2, "ECT Test : Check if ECT is not set on Syn, Syn+Ack and Ack but set on Data packets for non-DCTCP but ECN enabled traffic"),TestCase::QUICK);
    AddTestCase (new TcpDctcpMyCodePointsTest (3, "ECE Functionality Test: ECE should only be sent by receiver when it receives CE flags"),
                 TestCase::QUICK);
  }
};

static TcpDctcpMyTestSuite g_tcpdctcpTest; //!< static var for test initialization
